...
function gravarAno($codigoModelo,$codigoMarca,$codigoFipe,$anoModeloFipe,$combustivelFipe,$valorFipe,$modeloFipe ,$marcaFipe){
		try{
			$sql 				= "SELECT * FROM 
								   fp_ano 
								   WHERE 
			...
			if($procurarRegistro->rowCount() == 0){
				$sql				= "INSERT INTO fp_ano (codigo_modelo,codigo_marca,codigo_fipe,ano,combustivel,valor,marca,modelo) VALUES 
									  (:codigo_modelo,:codigo_marca,:codigo_fipe,:ano_modelo_fipe,:combustivel_fipe,:valor_fipe,:marca_fipe,:modelo_fipe)";
				$cadastrarAno 	= conectar()->prepare($sql);
				$cadastrarAno->bindValue(":codigo_modelo",$codigoModelo,PDO::PARAM_INT);
                                $cadastrarAno->bindValue(":codigo_marca",$codigoMarca,PDO::PARAM_INT);
				...
				$cadastrarAno->bindValue(":marca_fipe",$marcaFipe);
                                $cadastrarAno->bindValue(":modelo_fipe",$modeloFipe);
