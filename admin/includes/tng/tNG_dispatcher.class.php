...
class tNG_dispatcher {
	...
	function displayValidationRules() {
		$outRules = '';
		$outRules .= '<script src="'.$this->relPath.'includes/tng/scripts/FormValidation.js" type="text/javascript" language="javascript"></script>' . "\r\n";
		...
		if (isset($this->UnivalProps) && is_array($this->UnivalProps) && count($this->UnivalProps) > 0) {
			$outRules .= '<script type="text/javascript" language="javascript">';
			...
			if ('DEVELOPMENT' == $GLOBALS['tNG_debug_mode']) {
				...
				function needHelp() {
					if (confirm('Some data will be submitted to InterAKT. Do you want to continue?')) {
						...
						var wnd = window.open('{$this->relPath}includes/tng/pub/blank.html', 'KTDebugger_' + rand, '');						
						try {
						...
						frm.appendChild(utility.dom.createElement('INPUT', {'type': 'hidden', 'name': 'err', 'value': '{$js_err}'}, wnd));
						frm.appendChild(utility.dom.createElement('INPUT', {'type': 'hidden', 'name': 'devNotes', 'value': '{$js_devNotes}'}, wnd));
						frm.appendChild(utility.dom.createElement('INPUT', {'type': 'hidden', 'name': 'os', 'value': '{$js_os}'}, wnd));
						frm.appendChild(utility.dom.createElement('INPUT', {'type': 'hidden', 'name': 'webserver', 'value': '{$js_webserver}'}, wnd));
						frm.appendChild(utility.dom.createElement('INPUT', {'type': 'hidden', 'name': 'servermodel', 'value': '{$js_servermodel}'}, wnd));
						frm.appendChild(utility.dom.createElement('INPUT', {'type': 'hidden', 'name': 'installation', 'value': '{$js_installation}'}, wnd));
						frm.appendChild(utility.dom.createElement('INPUT', {'type': 'hidden', 'name': 'extensions', 'value': '{$js_extensions}'}, wnd));
						setTimeout(function() {
...
EOD;
				...
				if ($ret_devel != '') {
					$ret .= '<div id="KT_tngdeverror"><label>Developer Details:</label><div>'.$ret_devel.'</div><div id="KT_needhelp"><a href="javascript:needHelp()">'.KT_getResource('ONLINE_TROUBLESHOOT','tNG').'</a></div></div>';
				}
				$tmp = tNG_log::getResult('html');
				$ret .= '<div id="KT_tngtrace"><label>tNG Execution Trace - <a href="#" onclick="document.getElementById(\'KT_tngtrace_details\').style.display=(document.getElementById(\'KT_tngtrace_details\').style.display!=\'block\'?\'block\':\'none\'); return false;">VIEW</a></label>' . $tmp . '</div>';
			}
			if ("" != $GLOBALS['tNG_debug_log_type']) {
				$txtContent .= "\r\n\r\nDeveloper Details:\r\n  " . $ret_devel;
				$tmp = tNG_log::getResult('text');
				$txtContent .= "\r\n\r\ntNG Execution Trace:\r\n" . $tmp;
				...
				if ($GLOBALS['tNG_debug_log_type'] == 'logfile') {
					...
					$email->sendEmail($GLOBALS['tNG_email_host'], $GLOBALS['tNG_email_port'], $GLOBALS['tNG_email_user'], $GLOBALS['tNG_email_password'], $GLOBALS['tNG_debug_email_from'], $GLOBALS['tNG_debug_email_to'], "", "", $GLOBALS['tNG_debug_email_subject'], "utf-8", $txtContent, "");
				}
			}
		}
		return $ret;
