...
function KT_setServerVariables() {
	...
	if (!isset($GLOBALS['HTTP_COOKIE_VARS'])) {
		$GLOBALS['HTTP_COOKIE_VARS'] = &$_SESSION;
	}
...
function KT_Rel2AbsUrl($pageUrl, $templateUrl, $relUrl) {
	if (substr($relUrl,0,1) == "/") {
		return KT_getServerName().$relUrl;
	}
...
*/
function KT_transformsPaths($templateUrl, $text) {
	if ($templateUrl == "./" ) {
		$templateUrl = "";
	}
	
	$scriptUrl = KT_getUri();
	preg_match_all('/<(a|img|link|script|form|iframe|embed|input)([^>]*)>/ims',$text, $matches);
	foreach($matches[2] as $key=>$value) {
		preg_match('/\s(href|src|action)\s*=\s*(("((\\\"|[^"])+)")|(\'((\\\\\'|[^\'])+)\')|([^\s]+))?/ims' ,$value, $mmatch);
		if (isset($mmatch[2])) {
			$mydelim = '';
			...
			&& !($mmatch[1] == 'action' && $mmatch[2] == "")) {
				$text = preg_replace("/".$mmatch[1]."\s*=\s*".preg_quote($mydelim . $mmatch[2] . $mydelim, "/")."/ims", $mmatch[1].'="'.KT_Rel2AbsUrl($scriptUrl, $templateUrl, $mmatch[2]).'"', $text);
			}
		}
	}
	...
	if (preg_match("/UNI_navigateCancel/i", $text)) {
		...
		foreach($matches[1] as $key=>$value){
			$text = str_replace($value, KT_Rel2AbsUrl($scriptUrl, $templateUrl, $value), $text);
		}
	}
...
function KT_realpath($pathName, $isFolder = true) {
	...
	while (strpos($realPath, '../') !== false ) {	
		$pos = strpos($realPath, '../');
		$pos2 = strrpos(substr($realPath, 0, $pos - 1), '/');
		if ($pos2 !== false) {
			$realPath = substr_replace($realPath, '', $pos2, $pos - $pos2 + 2);
		} else {
			break;
		}
	}
 ...
 * returns string the date in the new format
 */
