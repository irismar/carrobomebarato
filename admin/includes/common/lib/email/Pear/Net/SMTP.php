...
require_once PEARDIR . 'Net/Socket.php';
...
class Net_SMTP
{
     ...
     * @since   1.0
     */
    function Net_SMTP($host = null, $port = null, $utf8 = null)
    {
        if (isset($host)) $this->host = $host;
        if (isset($port)) $this->port = $port;
        if (isset($utf8)) $this->utf8 = $utf8;
         ...
         */
        if ((@include_once PEARDIR . 'Auth/SASL.php') === false) {
    ...
    function _parseResponse($valid)
    {
        ...
        if (is_array($valid)) {
						foreach ($valid as $key => $valid_code) {
    ...
    function _negotiate()
    {
        if (PEAR::isError($error = $this->_put('EHLO', $this->utf8))) {
            return $error;
        }
        ...
        if (PEAR::isError($this->_parseResponse(250))) {
            ...
            if (PEAR::isError($error = $this->_put('HELO', $this->utf8))) {
                return $error;
            }
            if (PEAR::isError($this->_parseResponse(250))) {
    ...
    function _getBestAuthMethod()
    {
        $available_methods = explode(' ', $this->_esmtp['AUTH']);
				...
				foreach ($this->auth_methods as $key => $method) {
